...
- Integration order: deterministic fixture and local HTTP flow are complete; real Baidu map data and constrained real AI are implemented locally; live credentials, revision and hardening remain.
- No login, database, Redis, queue, server deployment, or overseas planning in the MVP.

## Current implementation

Python 兼容后端已经接管前端当前使用的本地接口：`GET /health`、`POST /api/chat` 和 `POST /api/trips/plan`。聊天接口通过 OpenAI-compatible Chat Completions 调用 AI，并支持 SSE 增量输出；规划接口在 `fixture` 模式返回南京三日 fixture，在 `full-real`/`map-real` 模式读取百度真实 POI、BD-09 坐标和路线，并要求 AI 只选择已验证 UID，最后校验路线端点和 StoryTimeline 身份。`GET /api/v1/health` 保留为 Python 规范健康检查。

默认 Adapter 模式仍为 `fixture`。真实模式需要 `LUMIVO_BAIDU_MAP_AK` 和 AI Key；实时凭据/配额验证、全国更丰富的检索、NDJSON 规划接口、修订、数据库和外部持久化仍未完成。

## Immediate build sequence

1. Verify live Baidu POIs/routes and live AI with configured credentials.
2. Expose the canonical NDJSON planning endpoint and connect a frontend TripClient.
3. Add stateless revision and hardening.
